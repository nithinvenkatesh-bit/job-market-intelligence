...
con = duckdb.connect()
con.execute("SELECT setseed(0.42)")  # reproducible sampling
...
# ---------------------------------------------------------------------------
# Benchmark: stratified on the two dimensions that actually drive difficulty
...
# Gold seed: rows you will hand-annotate for the fields with no free label
# (years of experience, required vs preferred skills, work arrangement).
con.execute(f"""
    CREATE OR REPLACE TABLE gold_seed AS
    SELECT * FROM benchmark USING SAMPLE {GOLD_N} ROWS
""")
